...
export async function GET(req: NextRequest) {
  ...
  const { data } = await supabaseAdmin
    .from('merchants')
    .select('id, store_name, widget_primary_color, widget_btn_text_color, widget_position, widget_offset_bottom, widget_offset_side, widget_title, points_per_dollar, signup_bonus, social_follow_url, follow_points, referral_points, tier_silver, tier_gold, tier_bronze_multiplier, tier_silver_multiplier, tier_gold_multiplier, tier_silver_bonus, tier_gold_bonus')
